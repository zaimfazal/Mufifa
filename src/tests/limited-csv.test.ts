...
const HEADER = 'match_id,stage,home_team,away_team,predicted_home_score,predicted_away_score,predicted_scorers_home,predicted_scorers_away'
...
describe('isLimitedCsvText', () => {
  it('detects the limited template by header', () => {
    expect(isLimitedCsvText(HEADER + '\nR16_089,Round of 16,Brazil,Spain,2,1,10;7,9')).toBe(true)
  })
...
describe('validateLimitedCsv', () => {
  ...
  it('rejects repeated stage labels even when predicted teams resemble a match', () => {
    const stageMatches = [
      { match_code: 'R16_001', home_team: 'Russia', away_team: 'Morocco', stage: 'round_of_16' },
      { match_code: 'R16_002', home_team: 'Argentina', away_team: 'Brazil', stage: 'round_of_16' },
    ]
    const rows = parseLimitedCsvText(
      HEADER + ',predicted_winner\n' +
      'Round of 16,Round of 16,Argentina,Brazil,2,1,9;10,7,Argentina\n' +
      'Round of 16,Round of 16,Russia,Morocco,0,1,,9,Morocco'
    ).rows

    const res = validateLimitedCsv(rows, stageMatches)
    expect(res.valid).toBe(false)
    expect(res.errors.some(e => e.message.includes('unique match code'))).toBe(true)
  })
})
