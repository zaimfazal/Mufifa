...
const HEADER = 'match_id,home_team,away_team,predicted_home_score,predicted_away_score,predicted_scorers_home,predicted_scorers_away,predicted_winner'
...
describe('parseJerseyNumbers', () => {
  it('parses a semicolon list into a sorted unique set', () => {
    expect(parseJerseyNumbers('10;7;7').numbers).toEqual([7, 7, 10]) // sort
...
describe('isLimitedCsvText', () => {
  it('detects the limited template by header', () => {
    expect(isLimitedCsvText(HEADER + '\nR16_089,Brazil,Spain,2,1,10;7,9,Brazil')).toBe(true)
  })
...
describe('validateLimitedCsv', () => {
  ...
  it('accepts a valid row and produces jersey scorer sets', () => {
    const rows = parse('R16_089,Brazil,Spain,2,1,10;7,9,Brazil')
  ...
  it('rejects when scorer count does not match the score', () => {
    const rows = parse('R16_089,Brazil,Spain,2,1,10,9,Brazil') // 2-0 home but only 1 scorer
    const res = validateLimitedCsv(rows, matches)
  ...
  it('accepts a 0-0 with no scorers', () => {
    const rows = parse('R16_089,Brazil,Spain,0,0,,,draw')
    const res = validateLimitedCsv(rows, matches)
    expect(res.valid).toBe(true)
  ...
  it('rejects non-integer jersey numbers', () => {
    const rows = parse('R16_089,Brazil,Spain,1,0,abc,,Brazil')
    const res = validateLimitedCsv(rows, matches)
    expect(res.valid).toBe(false)
  ...
  it('rejects an unknown match id', () => {
    const rows = parse('XX_999,A,B,1,0,10,,A')
    const res = validateLimitedCsv(rows, matches)
    expect(res.valid).toBe(false)
  ...
  it('flags missing predictions for matches in the bracket', () => {
    const rows = parse('R16_089,Brazil,Spain,1,0,10,,Brazil')
